...
function scene1_update(dt)
	...
	if not rockets or rockets[1].x > hund/2 then
		scrollx = scrollx + dt*hund/2
	end
	
	if rockets and rockets[1]:checkcol(playerx, playery) and rockets[1].x <= hund/2 then
...
function playermovement1(dt)
    ...
    for i, touch in ipairs(touches) do
        ...
        if i == 1 then
            if x/scale < playerx then
                playerx = math.max(0, playerx-dt*movement1speed)
            elseif x/scale > playerx then
                playerx = math.min(hund, playerx+dt*movement1speed)
            end
	        if y/scale < playery then
                playery = math.max(50, playery-dt*movement1speed)
            elseif y/scale > playery then
                playery = math.min(80, playery+dt*movement1speed)
            end
        elseif proshkas == 0 then
            scene1_action()
        end
    end
    proshkas = viskas
...
function scene1_draw()
	...
	for i = 1, backgroundstripes, 2 do
		...
		local point1 = {50*scale+hund*scale*math.cos(alpha), 64*scale+hund*scale*math.sin(alpha)}
		
		local alpha = math.rad(((i+1)/backgroundstripes + math.mod(sunrot/100, 1)) * 360)
		local point2 = {50*scale+hund*scale*math.cos(alpha), 64*scale+hund*scale*math.sin(alpha)}
	...
	for i = 1, 2 do
		draw(groundimg, -math.mod(scrollx, 100) + (i-1)*100, 58)
	end
	for i = 3, 4 do
		draw(groundimg, -math.mod(scrollx, 100) + (i-1)*100, 58)
	end
