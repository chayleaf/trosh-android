...
function scene4_draw()
	...
	for i = 1, backgroundstripes, 2 do
		...
		local point1 = {lastexplosion[1]*scale+hund*2*scale*math.cos(alpha), lastexplosion[2]*scale+hund*2*scale*math.sin(alpha)}
		
		local alpha = math.rad(((i+1)/backgroundstripes + math.mod(sunrot/100, 1)) * 360)
		local point2 = {lastexplosion[1]*scale+hund*2*scale*math.cos(alpha), lastexplosion[2]*scale+hund*2*scale*math.sin(alpha)}
	...
	if (starttimer > 0 and starttimer < 3) or alerttimer > 0.1 then
		...
		draw(alertimg, hund/2+math.random(5)-3, 40+math.random(5)-3, (math.random()*2-1)*0.1, i*0.5+0.6, i*0.5+0.6, 54, 15)
		draw(randomshitimg, hund/2+math.random(20)-10, 40+math.random(20)-10, 0, 1, 1, 50, 42)
	end
