...
function menu_draw()
	...
	for i = 1, 2 do
		draw(groundimg, -math.mod(scrollx, 120) + (i-1)*120, 58)
	end
	for i = 3, 4 do
		draw(groundimg, -math.mod(scrollx, 120) + (i-1)*120, 58)
	end
	...
	draw(titleimg, hund/2, 23, math.sin(sini)/10, (math.sin(sini2)+1)/5+0.7, (math.sin(sini2)+1)/5+0.7, 50, 13)
	
	love.graphics.setColor(255, 0, 0)
	if starti >= 0 then
		properprint("directed by maurice", hund*0.13, 40+textpos[0], scale/2)
	end
	if starti >= 1 then
		properprint("use finger to move", hund*0.25, 40+textpos[1], scale/2)
	end
	if starti >= 2 then
		properprint("and second finger to attack", hund*0.09, 40+textpos[2], scale/2)
	end
	if starti >= 3 then
		properprint("get ready...", hund*0.2, 40+textpos[3], scale/2)
	end
	if starti >= 4 then
		properprint("3", hund*0.4, 40+textpos[4], scale*2)
	end
	if starti >= 5 then
		properprint("2", hund*0.36, 40+textpos[5], scale*3)
	end
	if starti >= 6 then
		properprint("1", hund*0.32, 40+textpos[6], scale*4)
	end
	if starti >= 7 then
		properprint("go!", hund*0.1, 40+textpos[7], scale*6)
	end
