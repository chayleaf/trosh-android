...
function rocket:update(dt)
	if self.ground then
		if self.x > hund/2 then
			self.x = self.x - dt*(hund/2)
			if self.x < hund/2 then
				self.x = hund/2
			end
		end
	...
	if self.inflight then
	    ...
	    if viskas == 0 then
            ...
            for i, touch in ipairs(touches) do
                ...
                if i == 1 then
		            if x/scale > self.x and x/scale > self.x + 3 and self.x < hund then
			            self.speedx = self.speedx + dt*rocketspeed
		            elseif self.speedx > 0 then
			            self.speedx = math.max(0, self.speedx - dt*rocketspeed)
		            end
		    
		            if x/scale < self.x and x/scale < self.x - 3 and self.x > 0 then
			            self.speedx = self.speedx - dt*rocketspeed
		            elseif self.speedx < 0 then
			            self.speedx = math.min(0, self.speedx + dt*rocketspeed)
		            end
		...
		if wheatleytimer then
			...
			if wheatleytimer >= 0.95 and wheatleytimer - dt*2 < 0.95 then
				...
				rocketimg = love.graphics.newImage("rocketkaputt.png")
