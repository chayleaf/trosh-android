...
function scene5_update(dt)
	...
	if starttimer >= 25 and starttimer - dt < 25 then
		...
		landingx = math.random(hund-40)+20
	end
...
function scene5_draw()
	...
	if awesometimer > 0 then
		draw(awesomeimg, (awesometimer*2-1)*hund, 0)
		love.graphics.setColor(0, 0, 0)
		properprint("1000 points!", (awesometimer*2-1)*hund+3, 73, scale)
	...
	if landing then
		...
		draw(arrowimg, math.min(hund-8, landingx+(timeleft/landingtime)*80), 67, 0, -1, 1, 9)
	end
end
